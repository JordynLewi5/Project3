...
public class CanUserCancelOrderService {
    ...
    private static void serve(Socket socket, int clientID) throws Exception {
        ...
        UsernameOrderModel usernameOrder = new Gson().fromJson(msg, UsernameOrderModel.class);

        Class.forName("org.sqlite.JDBC");
        DataAccess adapter = new SQLiteDataAdapter();
        adapter.connect("jdbc:sqlite:shop.db");

        OrderModel model = adapter.loadOrder(usernameOrder.orderID);

        Boolean ans = false;
        if (model != null) {
            if (model.buyerName.equals(usernameOrder.username))
                ans = true;
            else
                ans = false;
        }
 ...
 */
    private static boolean register(String regHost, int regPort, String myHost, int myPort) throws IOException {
        ...
        info.serviceCode = ServiceInfoModel.CAN_USER_CANCEL_ORDER_SERVICE;
